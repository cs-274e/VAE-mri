...
Pipeline:
1. Use age-conditioned PairedNiftiSliceDataset on dataset/test (same preprocessing as training).
...
6. Save example prediction images per subject for z in {1, 11, 21, 31} (if available),
   each as its own PNG in a per-subject folder, with the same normalization,
   size, and orientation as your colleague's PNG dataset.
"""
...
#   Adjust the import path/module name below if needed.
# =====================================================
THIS_DIR = Path(__file__).resolve().parent         # .../conditional_ddpm/test
PROJECT_ROOT = THIS_DIR.parent                     # .../conditional_ddpm
sys.path.append(str(PROJECT_ROOT))

from train.train_conditional_ddpm_4_age_resume import (  # <-- CHANGE if filename differs
...
# Path to trained DDPM checkpoint (best weights)
CKPT_PATH = "conditional_ddpm/train/best_models/ddpm_cond_t1_to_t2_3_corrected_3_gpu_age.pth"
...
RESULTS_DIR = "conditional_ddpm/test/eval_outputs_corrected_3_age"
...
def sample_t2_from_t1_age(
    ...
    Args:
        ...
        age: (B,1) normalized ages in [0,1]
        device: torch.device
    ...
    for t in reversed(range(T)):
        ...
        eps_theta = model(model_in, t_batch, age)  # condition on time + age
...
def evaluate():
    ...
    # Dataset & DataLoader
    print(f"[Stage] Building test dataset from {TEST_ROOT} with ages from {AGE_CSV} ...")
    ...
    with torch.no_grad():
        for i, (t1, t2, age, meta) in enumerate(test_loader):
            ...
            t2 = t2.to(DEVICE)       # (B,1,H_model,W_model) not used directly
            ...
            # --- Generate T2 patch at model resolution (192x192) ---
            t2_pred_patch = sample_t2_from_t1_age(
                model,
                diffusion,
                t1,
                age,
            ...
            t2_pred_bbox = torch.nn.functional.interpolate(
                t2_pred_patch,
            ...
            if z_val in EXAMPLE_ZS and z_val not in examples[subj_name]:
                # full_pred_png_np is already full-FOV, PNG orientation, uint8
