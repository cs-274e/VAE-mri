#!/usr/bin/env python3
"""
Evaluation script for age-conditioned DDPM model on MRI T1->T2 translation.

Pipeline:
1. Use age-conditioned PairedNiftiSliceDataset on dataset/test.
2. Run conditional DDPM sampling to generate T2 | (T1, age) at IMAGE_SIZE x IMAGE_SIZE.
3. For each slice:
   - Resize the prediction back to the brain bounding box size.
   - Paste it into a full-FOV slice (original H_vol x W_vol).
...
6. Save example prediction images per subject for z in {1, 11, 21, 31} (if available),
   each as its own PNG in a per-subject folder, with the same size/orientation
...
# =====================================================
# IMPORT TRAINING COMPONENTS (adjust module name if needed)
# =====================================================
THIS_DIR = Path(__file__).resolve().parent         # .../conditional_ddpm/test
PROJECT_ROOT = THIS_DIR.parent                     # .../conditional_ddpm
sys.path.append(str(PROJECT_ROOT))

from train.train_conditional_ddpm_3_age import (  # <-- change if filename differs
...
AGE_CSV   = "dataset/meta_cleaned.csv"   # same CSV used in training

# Path to trained age-conditioned DDPM checkpoint
...
RESULTS_DIR = "conditional_ddpm/test/eval_outputs_corrected_2_no_mask_age"
...
def _torch_ssim(img1: torch.Tensor, img2: torch.Tensor) -> float:
    ...
    mu1 = F.conv2d(img1, window, padding=5)
    mu2 = F.conv2d(img2, window, padding=5)

    mu1_sq = mu1 * mu1
    mu2_sq = mu2 * mu2
    mu1_mu2 = mu1 * mu2

    sigma1_sq = F.conv2d(img1 * img1, window, padding=5) - mu1_sq
    sigma2_sq = F.conv2d(img2 * img2, window, padding=5) - mu2_sq
    sigma12 = F.conv2d(img1 * img2, window, padding=5) - mu1_mu2
...
def compute_metrics(img_gt: np.ndarray, img_pred: np.ndarray, resize_method: str = 'bicubic'):
    ...
    # MSE
    mse = F.mse_loss(img1_t, img2_t).item()
...
# =========================
# Age-conditioned DDPM Sampling: T2 | (T1, age)
# =========================
...
def sample_t2_from_t1_age(
    ...
    """
    Run full reverse diffusion to sample T2 given T1 and age.

    Args:
        model: UNetConditional (time + age conditioned)
        diffusion: DiffusionConfig
    ...
    for t in reversed(range(T)):
        ...
        eps_theta = model(model_in, t_batch, age)  # age-conditioned
...
def evaluate():
    ...
    # Dataset & DataLoader
    print(f"[Stage] Building age-conditioned test dataset from {TEST_ROOT} ...")
    ...
    # Model input size
    t1_sample, _, age_sample, _ = test_dataset[0]
    _, H_model, W_model = t1_sample.shape
    print(f"Evaluating age-conditioned DDPM with input size {H_model}x{W_model}")
    print(f"Example normalized age from dataset: {age_sample.item():.3f}")
    ...
    with torch.no_grad():
        for i, (t1, t2, age, meta) in enumerate(test_loader):
            if i % 10 == 0:
                print(f"Processing slice batch {i+1}/{len(test_loader)}...")

            t1 = t1.to(DEVICE)    # (B,1,H_model,W_model)
            t2 = t2.to(DEVICE)    # (B,1,H_model,W_model) (unused directly)
            ...
            subj_name = subj_path.name  # e.g. "s0001"
            ...
            # --- Generate T2 patch at model resolution (192x192), age-conditioned ---
            t2_pred_patch = sample_t2_from_t1_age(model, diffusion, t1, age, DEVICE)  # (1,1,H_model,W_model)
            ...
            # Resize prediction back to brain bbox size
            t2_pred_bbox = F.interpolate(
                t2_pred_patch,
            ...
            # Create full-FOV slice (H_vol x W_vol) in NIfTI orientation
            full_pred = torch.zeros(1, 1, H_vol, W_vol, device=DEVICE)
            full_pred[:, :, y_min:y_max+1, x_min:x_max+1] = t2_pred_bbox

            # Convert to numpy [0,255] in original NIfTI orientation
            full_pred_np = tensor_to_numpy(full_pred)  # (H_vol, W_vol)
            ...
            if SAVE_INDIVIDUAL_METRICS:
                individual_metrics.append({
                    ...
                    'age_norm': float(age.cpu().numpy().ravel()[0]),
                })
    ...
    print("\n" + "=" * 60)
    print("EVALUATION RESULTS (Age-conditioned DDPM T1->T2, full-FOV PNG)")
    print("=" * 60)
    ...
    with open(metrics_file, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EVALUATION RESULTS (Age-conditioned DDPM T1->T2, full-FOV PNG)\n")
        f.write("=" * 60 + "\n")
        f.write(f"Checkpoint: {CKPT_PATH}\n")
        f.write(f"NIfTI test root: {TEST_ROOT}\n")
        f.write(f"Age CSV: {AGE_CSV}\n")
