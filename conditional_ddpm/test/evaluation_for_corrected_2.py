...
import matplotlib.pyplot as plt  # still imported, though we no longer use subplots
...
from train.train_conditional_ddpm_3 import (  # adjust if filename differs
...
# Path to trained DDPM checkpoint
...
RESULTS_DIR = "conditional_ddpm/test/eval_outputs_corrected_2"
...
@torch.no_grad()
def sample_t2_from_t1(model: UNetConditional, diffusion: DiffusionConfig,
                      t1: torch.Tensor, device: torch.device) -> torch.Tensor:
    """
...
def evaluate():
    ...
    print("\n" + "=" * 60)
    print("EVALUATION RESULTS (DDPM T1->T2, full-FOV PNG)")
    print("=" * 60)
    ...
    with open(metrics_file, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EVALUATION RESULTS (DDPM T1->T2, full-FOV PNG)\n")
        f.write("=" * 60 + "\n")
